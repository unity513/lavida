...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and indexname = 'marketplace_orders_idempotency_key_unique_idx'
  ) and not exists (
    select 1
    ...
    create unique index marketplace_orders_idempotency_key_unique_idx
...
alter table public.crypto_order_idempotency_keys enable row level security;

drop policy if exists "Admins read crypto order idempotency keys" on public.crypto_order_idempotency_keys;
create policy "Admins read crypto order idempotency keys"
  on public.crypto_order_idempotency_keys for select
  using (public.lavida_is_marketplace_admin());

do $$
declare
  v_target_asset_id uuid := '7d360766-3a2c-4332-8d84-483c9b05b282';
  v_target_product_id uuid := '790de765-cbc8-4356-ab09-697ae35e20e8';
begin
  if exists (
    select 1
    from public.crypto_assets
    where id = v_target_asset_id
      and marketplace_product_id = v_target_product_id
      and upper(asset_symbol) = 'USDT'
  ) and not exists (
    select 1
    from public.crypto_asset_networks
    where asset_id = v_target_asset_id
  ) then
    insert into public.crypto_asset_networks (
      asset_id,
      network_name,
      network_code,
      is_enabled,
    ...
    join public.crypto_assets ca
      on ca.id = n.asset_id
    where ca.id <> v_target_asset_id
      and upper(ca.asset_symbol) = 'USDT'
      and n.is_enabled = true
    order by n.network_code, n.sort_order, n.network_name
    on conflict (asset_id, network_code) do update
