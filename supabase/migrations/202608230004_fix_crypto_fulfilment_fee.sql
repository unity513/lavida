...
create table if not exists public.crypto_order_idempotency_keys (
...
begin
  ...
  where c.conrelid = 'public.marketplace_orders'::regclass
    and c.conname = 'marketplace_orders_fulfilment_method_check';

  if v_existing_expr is null then
    v_existing_expr := 'fulfilment_method = any (array[''pickup''::text, ''delivery''::text, ''digital''::text])';
  end if;

  execute 'alter table public.marketplace_orders drop constraint if exists marketplace_orders_fulfilment_method_check';
  execute format(
    'alter table public.marketplace_orders add constraint marketplace_orders_fulfilment_method_check check ((%s) or fulfilment_method = %L)',
    v_existing_expr,
    'digital'
  );
end $do$;
...
declare
  ...
  v_service_fee_mwk numeric := 0;
  v_service_fee_usdt numeric := 0;
  v_service_fee_label text := 'Service fee';
...
begin
  ...
  v_service_fee_mwk := coalesce(
    nullif(v_fee ->> 'fee_mwk', '')::numeric,
    nullif(p_order #>> '{service_fee,fee_mwk}', '')::numeric,
    0
  );
  v_service_fee_usdt := coalesce(
    nullif(v_fee ->> 'fee_usdt_equivalent', '')::numeric,
    nullif(p_order #>> '{service_fee,fee_usdt_equivalent}', '')::numeric,
    0
  );
  v_service_fee_label := coalesce(
    nullif(v_fee ->> 'label', ''),
    nullif(p_order #>> '{service_fee,label}', ''),
    'Service fee'
  );
  v_requires_admin_review := coalesce(
    nullif(v_fee ->> 'requires_admin_review', '')::boolean,
    nullif(p_order #>> '{service_fee,requires_admin_review}', '')::boolean,
    false
  );
  v_total := round(v_crypto_value + coalesce(v_service_fee_mwk, 0), 2);
  ...
  values (
    ...
    v_total, v_requires_admin_review, 'pending_payment'
  )
  ...
  return jsonb_build_object(
    ...
    'service_fee_label', v_service_fee_label,
    'requires_admin_review', v_requires_admin_review,
